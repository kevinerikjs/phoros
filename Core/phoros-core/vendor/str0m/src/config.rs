...
pub struct RtcConfig {
    ...
    pub(crate) nack_min_interval: Duration,
...
impl RtcConfig {
    ...
    /// Minimum time between NACK reports (default 33 ms). A low-latency, low-loss link
    /// wants a lost packet asked for at once; a phoros patch, not upstream.
    pub fn set_nack_min_interval(mut self, interval: Duration) -> Self {
        self.nack_min_interval = interval;
        self
    }
...
impl Default for RtcConfig {
    fn default() -> Self {
        Self {
            ...
            stats_interval: None,
            nack_min_interval: Duration::from_millis(33),
